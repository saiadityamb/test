...
function Call_Clicks_Estimation(){
    let CallClicks_New_Estimation = firebase.database().ref("Call_Clicks");
    CallClicks_New_Estimation.update({
               amount_estimation:window.callclicks_estimation+1,   
});
// animations = false
}
...
let CallClicks_Estimation = firebase.database().ref("Call_Clicks").child("amount_estimation");
CallClicks_Estimation.on('value', (snapshot) => {
    const data_estimation = snapshot.val();
    window.callclicks_estimation = data_estimation;
});
...
setTimeout(function(){
    document.getElementById("scroll_after_submit").style.display = "block"
},500)

setTimeout(function(){
    document.getElementById('scroll_after_submit').scrollIntoView({
        behavior: 'smooth'
      });
    document.getElementById("Main_Image").style.display = "none"
    document.getElementById("pincode_form").style.display = "none"
    document.getElementById('scroll_after_submit').scrollIntoView({
        behavior: 'smooth'
      });
    // let pincode = document.getElementById("Number").value
    // httpGet(`https://connect.pabbly.com/workflow/sendwebhookdata/IjU3NjEwNTY0MDYzNTA0Mzc1MjY5NTUzYyI_3D_pc?pincode = ${pincode}`)
...
//         document.getElementById("Number").classList.remove("animate__animated")
//         document.getElementById("Number").classList.remove("animate__heartBeat")
//     }
// setTimeout(function(){
//     if(animations){

//         document.getElementById("Number").classList.add("animate__animated")
//         document.getElementById("Number").classList.add("animate__heartBeat")
//     }
// },1000);
...
setInterval(function(){
    ...
    if(animations){
        document.getElementById("Mobile_Number_1").classList.remove("animate__animated")
        document.getElementById("Mobile_Number_1").classList.remove("animate__heartBeat")
    }
setTimeout(function(){

    if(animations){
        document.getElementById("Mobile_Number_1").classList.add("animate__animated")
        document.getElementById("Mobile_Number_1").classList.add("animate__heartBeat")
    }
},1000);

...
function Call_Clicks(){
    let CallClicks_New = firebase.database().ref("Apartments_Enquiry_1").child("Call_Clicks");
...
let SticyCallButton = firebase.database().ref("Apartments_Enquiry_1").child("Sticky-Call-Button");
...
let CallClicks = firebase.database().ref("Apartments_Enquiry_1").child("Call_Clicks").child("amount");
...
let MobileNumber = firebase.database().ref("Apartments_Enquiry_1/Content_Section").child("Tagline");
...
function Submit_Details(){
    ...
    if(name!="" && email!="" && mobno!=""){
        const url = `https://connect.pabbly.com/workflow/sendwebhookdata/IjU3NjIwNTZkMDYzMjA0M2Q1MjY5NTUzNiI_3D_pc?Name=${name}&Email=${email}&Number=${mobno}&Title=Property_1`
